...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   display_assets_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: aradice <aradice@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/07/19 00:05:01 by aradice           #+#    #+#             */
/*   Updated: 2022/07/20 04:54:04 by aradice          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_place_assets_enemy(t_data_all *data, int x, int y, void *enemy)
{
	if (data->map[x][y] == 'F')
		mlx_put_image_to_window(data->mlx_ptr,
			data->window_ptr, enemy, y * 60, x * 60);
}
...
void	ft_display_assets(t_data_all *data)
{
	...
	ft_display_movements_count(data);
}
...
void	ft_display_animation(t_data_all *data, void *enemy)
{
	...
	while (data->map[x])
	{
		...
		while (data->map[x][y])
		{
			ft_place_assets_enemy(data, x, y, enemy);
			y++;
		}
		x++;
	}
